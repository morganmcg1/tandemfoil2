...
# Usage: bash scripts/launch_round8.sh <gpu> <name> [extra args...]
set -u
...
GPU="$1"; shift
NAME="$1"; shift
LOGFILE="logs/round8/${NAME}.log"
echo "[GPU $GPU] $NAME (epochs=$EPOCHS, timeout=${TIMEOUT}min) → $LOGFILE"
CUDA_VISIBLE_DEVICES=$GPU SENPAI_TIMEOUT_MINUTES=$TIMEOUT \
  nohup python ./train.py "${COMMON[@]}" --wandb_name "${GROUP}/r8-${NAME}" "$@" \
  > "$LOGFILE" 2>&1 &
sleep 5
PID=$(ps -eo pid,ppid,cmd --no-headers | grep "wandb_name ${GROUP}/r8-${NAME}" | grep -v grep | awk '$2==1 {print $1}' | head -1)
echo "  main PID=$PID"
echo "$PID" > "logs/round8/${NAME}.pid"
