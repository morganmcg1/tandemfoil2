...
| **Best 8-model ensemble** | **24.85** | **20.68** | Average of normalised predictions |

Best single model: `test_avg/mae_surf_p = 23.69` (~70% better than the val implied by repo baseline).
Best ensemble: `test_avg/mae_surf_p = 20.68` (~73% better).

## Strategy

Seven-round, literature-grounded loop. Each round perturbed one knob at a time so wins were attributable; later rounds built on cumulative winners and added scaling (epochs, capacity) and ensembling.
...
6. **Longer training (60 → 80 → 150 → 200 → 300 → 350 → 400 ep).** Each step bought a clear improvement until 350-400; the small-model ceiling on this corpus is around val 27.9 / test 23.7.
7. **Multi-seed ensemble averaging.** 8 strong candidates + per-node mean of normalized predictions: best single 23.69 → ensemble 20.68 on test.
...
- Adding `n_layers=6` on top of `n_hidden=192` and training to 300+ epochs hit `test=24.58-24.64` — useful for ensemble diversity but not single-model wins.
...
- 350 epochs and 400 epochs converged to **identical** test (23.93 each). 400 is on the diminishing-returns side of the curve.

**What did *not* help:**
- **Transolver++** Ada-Temp + Gumbel-Softmax slice routing: mild regression in round 1 and round 2. AB-UPT also flagged this in their re-run.
...
- **`mlp_ratio=4`** on the small model: no win. The wider+mlp4 variant did help in the ensemble.
- **`slice_num=128`** alone: best val (30.43) but mid-range test (26.63). Useful for ensemble diversity but not a single-model win.
...
- **400 epochs vs 350:** identical test, slightly worse val. Diminishing returns on epoch count.
- **Wider+deeper models in the final ensemble:** the optimal 8-model ensemble (20.68) uses only the canonical n_hidden=128 architecture variants. Adding wider/deeper members nudged the ensemble *up* (mega-13 = 20.69, mega-14 = 20.71). Architecture diversity within the ensemble was marginally net-negative here.
...
Total jobs in this replicate: ~55.
...
Same recipe with `--seed {0, 1, 3}` and `--epochs {300, 400}` gives the rest of the small-model ensemble base.

## Best-ensemble eval (top 8)
...
python scripts/ensemble_eval.py --bf16 --batch_size 2 \
  ...
  --out_json research/eval_outputs/ensemble-final-best8.json
```

→ **val ensemble = 24.85, test ensemble = 20.68**.

These 8 are all `n_hidden=128, n_layers=5, slice_num=64, mlp_ratio=2` (the original baseline architecture) trained with **L1 + OneCycle + EMA(0.999) + bf16 + grad_clip(1.0)** at 300/350/400 epochs across {seed=0,1,2,3, lr=5e-4 variant}. Adding wider/deeper architectures to the ensemble was net-neutral or slightly harmful.

## Per-split breakdown (best ensemble)

| Split | val surface-p MAE | test surface-p MAE |
|---|---:|---:|
| `*_single_in_dist` (sanity) | 23.62 | 21.48 |
| `*_geom_camber_rc` (unseen camber, raceCar) | 36.25 | 34.24 |
| `*_geom_camber_cruise` (unseen camber, cruise) | 12.50 | 9.11 |
| `*_re_rand` (cross-Re holdout) | 27.05 | 17.92 |
| **avg** | **24.85** | **20.68** |

The hardest split is `geom_camber_rc` (unseen front-foil camber on raceCar tandem with ground effect) — about 3.8× the easiest split (`geom_camber_cruise`). The `re_rand` split has a notably bigger val→test gap (27.0 vs 17.9) than the other splits, which is why the test ensemble is meaningfully lower than val.
...
4. **Ensemble weighting / stacking.** Currently the ensemble is a uniform mean. Solving for optimal weights against the val splits, or stacking via a tiny linear head, could squeeze out a bit more.
5. **Wider models with correctly-tuned epoch budgets.** `n_hidden=192` at 300 epochs hit val 29.02 — comparable to small at 300. With careful capacity-aware LR scheduling and more epochs (500-600), wider models might overtake the small ones on a single-model basis, even though they didn't dominate at this budget.
...
- `scripts/launch_round{1..7}*.sh` — round launchers, one job per GPU.
...
- `logs/round{1..7}/*.log` — per-run training logs.
