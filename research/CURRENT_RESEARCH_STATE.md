...
| #580 | askeladd | lion-lr-1p2e-4 | `lr_lion = 1.7e-4 → 1.2e-4` on merged #535 baseline | **Sent back 06:45 for rebase + re-run**: clean win on run base #535 (val=55.547, −9.69 %); appendix-grade asymmetric-basin finding (downward sensitivity −0.33 %, upward +0.09 % per % lr). All 8 splits improve, cruise wins most. vs current #571 baseline 52.116 it's +6.58 % (past close threshold). **Squash CONFLICTS** on lr line (askeladd lr=1.2e-4 vs advisor lr=2.5e-4). Re-run on post-#571 maps lower edge under β2=0.999 (currently unmapped); pairs with tanjiro's #592 upper-edge probe to lock the new-baseline basin. Re-run band −5 % to +25 % (two competing hypotheses on basin shift under β2=0.999). |
