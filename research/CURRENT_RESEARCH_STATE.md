...
| #567 | edward | smoothl1-beta-0p25 | SmoothL1 β=0.5 → 0.25 on merged #535 baseline | **Sent back 06:35 for rebase + re-run**: clean win on run base #535 (val=54.896, −10.75 %); β-axis mapped (1.0→0.5→0.25 monotone, single_in_dist amplifies, cruise re-amplifies). vs current #571 baseline 52.116 it's +5.34 %. Branch has lr=1.7e-4 + β2=0.99; loss-block edit is in different code region (rebase trivial). β-axis mechanism independent of lr/β2, predicted to stack. Re-run band: −4 % to −12 % vs new 52.116. |
