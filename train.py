...
def evaluate_split(model, loader, stats, surf_weight, surf_p_weight, device) -> dict[str, float]:
    ...
    ch_w = torch.tensor([1.0, 1.0, surf_p_weight], device=device)
    ...
    with torch.no_grad():
        for x, y, is_surface, mask in loader:
            ...
            surf_num = (abs_err * ch_w * surf_mask.unsqueeze(-1)).sum()
            surf_den = (surf_mask.sum() * ch_w.sum()).clamp(min=1)
            surf_loss_sum += (surf_num / surf_den).item()
            n_batches += 1
...
class Config:
    ...
    surf_p_weight: float = 5.0   # multiplier on surface pressure channel inside surf_loss
    epochs: int = 50
...
for epoch in range(MAX_EPOCHS):
    ...
    for x, y, is_surface, mask in tqdm(train_loader, desc=f"Epoch {epoch+1}/{MAX_EPOCHS}", leave=False):
        ...
        ch_w = torch.tensor([1.0, 1.0, cfg.surf_p_weight], device=pred.device)
        surf_num = (abs_err * ch_w * surf_mask.unsqueeze(-1)).sum()
        surf_den = (surf_mask.sum() * ch_w.sum()).clamp(min=1)
        surf_loss = surf_num / surf_den
    ...
    model.eval()
    split_metrics = {
        name: evaluate_split(model, loader, stats, cfg.surf_weight, cfg.surf_p_weight, device)
