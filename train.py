...
def evaluate_split(model, loader, stats, surf_weight, device, bf16: bool = False) -> dict[str, float]:
    ...
    n_surf = n_vol = n_batches = n_skipped = 0
    ...
    with torch.no_grad():
        for x, y, is_surface, mask in loader:
            ...
            # Per-node finite check on pred (bf16 numerics) AND y (dataset
            # corruption — e.g. test_geom_camber_cruise sample 20 has inf in y).
            # Per-sample skip alone is not enough: the 0*NaN IEEE trap inside
            # accumulate_batch poisons channel-wise MAE accumulators.
            pred_finite = torch.isfinite(pred).all(dim=-1)  # [B, N]
            y_finite = torch.isfinite(y).all(dim=-1)        # [B, N]
            node_finite = pred_finite & y_finite
            n_bad = int((mask & ~node_finite).sum().item())
            if n_bad:
                n_skipped += n_bad
            mask = mask & node_finite
            # Replace non-finite values with 0 so 0*X never produces NaN.
    ...
    if n_skipped:
        print(f"  [evaluate_split] WARNING: {n_skipped} non-finite nodes excluded from accumulators")
...
class Config:
    ...
    loss: str = "l1"              # "mse" | "l1"
    bf16: bool = True             # autocast forward pass to bfloat16
    n_layers: int = 1             # Transolver depth
    n_hidden: int = 128
    n_head: int = 4
    slice_num: int = 64
    mlp_ratio: int = 2
    clip_grad_norm: float = 1.0   # 0.0 disables clipping
    ema_decay: float = 0.995      # 0.0 disables EMA
...
if best_metrics:
    ...
    test_model = Transolver(**model_config).to(device)
    ...
    if not cfg.skip_test:
        ...
        test_metrics = {
            name: evaluate_split(test_model, loader, stats, cfg.surf_weight, device, bf16=cfg.bf16)
