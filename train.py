...
def evaluate_split(model, loader, stats, surf_weight, huber_delta, device) -> dict[str, float]:
    ...
    with torch.no_grad():
        for x, y, is_surface, mask in loader:
            ...
            huber_err = F.huber_loss(pred, y_norm, reduction='none', delta=huber_delta)
...
class Config:
    ...
    epochs: int = 50
    huber_delta: float = 1.0  # threshold between L2 and L1 regimes (normalized space)
...
for epoch in range(MAX_EPOCHS):
    ...
    model.eval()
    split_metrics = {
        name: evaluate_split(model, loader, stats, cfg.surf_weight, cfg.huber_delta, device)
